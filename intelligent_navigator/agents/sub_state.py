...
from intelligent_navigator.agents.prompts import PROMPT_PAGE_EXPLORER_SUBSTATES
...
class SubStateExplorer:
    ...
    def explore(
        ...
        base_selector_json = snapshot.selector_map_json
        ...
        for trigger in triggers:
            ...
            if not self._is_safe_trigger(element_index, base_selector_json):
                continue
    ...
    def _is_safe_trigger(self, element_index: int, selector_map_json: str) -> bool:
        """Safety check: ensure the trigger is not a submit/delete/logout button."""
        try:
    ...
    def _interact_with_trigger(
        ...
        try:
            ...
            new_links = self._extract_new_links_from_sub_state(
                base_selector_json, new_map_json
            )
            ...
            # Always dismiss overlays (Radix dropdowns, popovers, etc.)
            try:
                page = self.browser_controller.browser_context.get_current_page()
                if page:
    ...
    def _restore_page_state(
        ...
        try:
            ...
            # Always press Escape to dismiss any leftover overlay
            # (Radix Select popover, dropdown, etc.)
            page.keyboard.press("Escape")
            page.wait_for_timeout(300)
