"""
Credential parser for the Exploration Agent.
Extracts structured credentials from a credentials file using LLM.
Login/logout are handled naturally by the LLM exploration loop.
"""
...
from test_case_enhancer.agent.exploration_agent.models import RoleCredentials
from test_case_enhancer.agent.exploration_agent.prompts import PROMPT_CREDENTIAL_PARSING
...
class CredentialParser:
    ...
    def __init__(self, llm_client, credential_reader):
    ...
    def parse_credentials(self, credentials_file_path: str) -> List[RoleCredentials]:
        ...
        content = self.credential_reader.read_credentials_file(credentials_file_path)
    ...
    def deduplicate_roles(self, credentials: List[RoleCredentials]) -> List[RoleCredentials]:
        """Keep only one account per unique role name (case-insensitive)."""
        seen_roles: set = set()
    ...
    def sort_by_privilege(self, credentials: List[RoleCredentials]) -> List[RoleCredentials]:
        """Sort by privilege level descending (most privileged first)."""
    ...
    def format_credentials_for_prompt(self, credentials: List[RoleCredentials]) -> str:
        """Format credentials into a string suitable for the LLM exploration prompt."""
        if not credentials:
