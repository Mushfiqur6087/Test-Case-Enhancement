"""
Pure DOM-based link extraction for the Exploration Agent.
No LLM calls — the exploration LLM handles link classification and prioritization.
"""
...
class LinkExtractor:
    """Extracts outgoing links from a page using pure DOM parsing. No LLM calls."""
    ...
    def extract_links(
        ...
        The LLM receives these raw links and decides what to explore.
        """
        ...
        for idx_str, elem in elements.items():
            ...
            # Extract links from <a> elements with href
            if tag == "a" and href:
                resolved = self.identity_computer.resolve_url(href, page_url)

                # Skip external, same-page anchors, and javascript: links
