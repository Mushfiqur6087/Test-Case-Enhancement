...
class Tools:
    ...
    def _get_page_info(self, browser_context: BrowserSession) -> Dict[str, Any]:
        ...
        Includes a DOM stabilization wait for SPAs: if the DOM appears to be
        mid-transition (e.g. spinner text, loading indicators), we wait briefly
        for React/Vue/Angular to finish rendering before capturing.
        """
        try:
            ...
            # Wait for DOM to stabilize — SPAs may still be rendering after
            # a URL change (React Router updates URL before render completes)
            try:
            ...
            # Force-refresh the DOM tree
            browser_context._parser = None
            browser_context._selector_map = None
