...
class BrowserController:
    ...
    def click_element_by_index(self, element_index: int) -> bool:
        try:
            ...
            if page is None:
                print(f"[click_element] Failed: no current page")
                return False

            selector_map = self.browser_context.get_selector_map(refresh=True)
            if not selector_map or element_index not in selector_map:
                print(f"[click_element] Failed: element index {element_index} not in selector map (map size: {len(selector_map) if selector_map else 0})")
                return False
            ...
            # Remember URL before click to detect SPA navigation
            url_before = page.url
            ...
            # For submit buttons or form-related clicks, add extra wait for SPA
            # rendering (React/Vue/Angular may need time to process API response
            # and re-render the DOM after form submission)
            if el_type == "submit" or (tag == "button" and el_type == "submit"):
                page.wait_for_timeout(2000)
                # If URL changed (SPA navigation), wait more for rendering
                if page.url != url_before:
                    ...
                    try:
                        ...
                        pass
                    # Force clear DOM caches so next read gets fresh content
            ...
            print(f"[click_element] Exception clicking element {element_index}: {e}")
            return False
    ...
    def scroll_down(self, amount: int = 500) -> bool:
        ...
        try:
            ...
            page.wait_for_timeout(500)  # Brief wait for content to load
            # Clear parser & selector_map cache so DOM is re-read
            self.browser_context._parser = None
            self.browser_context._selector_map = None
            return True
        except Exception as e:
    ...
    def get_available_actions(self) -> list:
        ...
        if page is not None:
            ...
            # Scrolling is always available when a page is open
            available.append("scroll_down")
            available.append("scroll_up")
   ...
   Format: {"scroll_down": {"amount": number}}
   Example: {"scroll_down": {"amount": 500}}
   Note: amount is in pixels (default 500). Use this to see elements below the current viewport.""",

            "scroll_up": """scroll_up: Scroll the page up to see previous content
   Format: {"scroll_up": {"amount": number}}
   Example: {"scroll_up": {"amount": 500}}
   Note: amount is in pixels (default 500). Use this to see elements above the current viewport.""",
