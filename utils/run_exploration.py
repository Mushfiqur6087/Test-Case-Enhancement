"""
CLI entry point for the LLM-Guided Exploration Agent.

Usage:
    python -m test_case_enhancer.run_exploration \
...
def main():
    parser = argparse.ArgumentParser(
        description="LLM-Guided Exploration Agent - builds a navigation graph"
    )
    parser.add_argument(
    ...
    from test_case_enhancer.agent.exploration_agent.agent import ExplorationAgent
    ...
    try:
        ...
        print(f"LLM calls used: {result.exploration_stats.get('llm_calls', 0)}")
        ...
        try:
            agent.browser_controller.close()
        except Exception:
            pass
